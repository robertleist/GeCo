...
from src.utils.box_ops import compute_location, BoxList
from src.utils.data import FSC147Dataset
from src.utils.arg_parser import get_argparser
from src.utils.losses import ObjectNormalizedL2Loss, Detection_criterion
...
def train(args):
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for img, bboxes, img_name, gt_bboxes, density_map in train_loader:
            ...
            targets = BoxList(gt_bboxes, (args.image_size, args.image_size), mode='xyxy').to(device).resize(
                (512, 512))
        ...
        with torch.no_grad():
            ...
            for img, bboxes, img_name, gt_bboxes, density_map in val_loader:
                ...
                val_rmse += torch.pow(
                    density_map.flatten(1).sum(dim=1) - centerness.flatten(1).sum(dim=1), 2
                ).sum()
        ...
        if rank == 0:
            ...
            torch.save(
                checkpoint,
                os.path.join(args.model_path, f'{args.model_name}_last.pth')
            )
